...
function startTimer() {
    ...
    isPlaying = true;
    document.getElementById('player-view').classList.add('timer-running'); // [C]
    ...
    playBtn.style.backgroundColor = 'rgba(255,255,255,0.85)';
...
function pauseTimer() {
    isPlaying = false;
    document.getElementById('player-view').classList.remove('timer-running'); // [C]
    const playBtn = document.getElementById('player-play-btn');
...
function stopTimer() {
    ...
    if (playerView) playerView.classList.remove('timer-running'); // [C]
    const playBtn = document.getElementById('player-play-btn');
    if (playBtn) {
        playBtn.innerHTML = '<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"></path></svg>';
        playBtn.style.backgroundColor = '#FFFFFF';
